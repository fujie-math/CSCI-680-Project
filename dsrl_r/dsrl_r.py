...
SelfDSRL = TypeVar("SelfDSRL", bound="DSRL_R")


class DSRL_R(OffPolicyAlgorithm):
	"""
	...
	def _setup_model(self) -> None:
		...
		self.noise_space = spaces.Box(
			low=-self.action_magnitude*np.ones(self.diffusion_act_dim*self.diffusion_act_chunk),
			high=self.action_magnitude*np.ones(self.diffusion_act_dim*self.diffusion_act_chunk),
			dtype=np.float32
		)
	...
	def train(self, gradient_steps: int, batch_size: int = 64) -> None:
		...
		for gradient_step in range(gradient_steps):
			...
			with th.no_grad():
				...
				next_actions = th.tensor(self.policy_noise.unscale_action(next_actions.cpu().numpy())).to(self.device)
	...
	def _get_torch_save_params(self) -> tuple[list[str], list[str]]:
		state_dicts = ["policy", "policy_noise", "actor.optimizer", "critic.optimizer", "critic_noise"]
		if self.ent_coef_optimizer is not None:
	...
	def _sample_action(
		...
		if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
			# Warmup phase
			unscaled_action = np.array([self.noise_space.sample() for _ in range(n_envs)])
		else:
			# Note: when using continuous actions,
			# we assume that the policy uses tanh to scale the action
			# We use non-deterministic action in the case of SAC, for TD3, it does not matter
			assert self._last_obs is not None, "self._last_obs was not set"
			unscaled_action, _ = self.policy_noise.predict(observation=self._last_obs, deterministic=False)

		# Rescale the action from [low, high] to [-1, 1]
		if isinstance(self.action_space, spaces.Box):
			scaled_action = self.policy_noise.scale_action(unscaled_action)
			...
			action = self.policy_noise.unscale_action(scaled_action)
		else:
	...
	def predict_diffused(
		...
		unscaled_action, predict_second_return = self.policy_noise.predict(observation, state, episode_start, deterministic)
		if isinstance(self.action_space, spaces.Box):
			scaled_action = self.policy_noise.scale_action(unscaled_action)
			# We store the scaled action in the buffer
			buffer_action = scaled_action
