...
SelfDSRL = TypeVar("SelfDSRL", bound="DSRL_RES")


class DSRL_RES(OffPolicyAlgorithm):
	"""
	...
	def train(self, gradient_steps: int, batch_size: int = 64) -> None:
		...
		for gradient_step in range(gradient_steps):
			...
			with th.no_grad():
				batch_size_tensor = replay_data.next_observations.shape[0]
				zero_input = th.zeros(
					batch_size_tensor,
				...
				base_actions = self.diffusion_policy(
					replay_data.next_observations,
					zero_input,
					return_numpy=False,
				)
				base_actions = base_actions.reshape(-1, self.diffusion_act_chunk * self.diffusion_act_dim)

				# 2) actor 不再输出“完整动作”，而是输出残差 residual
				residual, next_log_prob = self.actor.action_log_prob(replay_data.next_observations)

				# 3) 残差直接加在 base_actions 上，并截断到 [-1, 1]
				next_actions = (base_actions + residual * self.res_scale).clamp(-1.0, 1.0)
			...
			if gradient_step in actor_gradient_idx:
				...
				# 3) 残差直接加在 base_actions 上，并截断到 [-1, 1]
				next = (base + actions_pi * self.res_scale).clamp(-1.0, 1.0)
	...
	def update_noise_critic(self, replay_data):
		with th.no_grad():
			zero_input = th.zeros(replay_data.actions.shape[0], self.diffusion_act_chunk, self.diffusion_act_dim, device=self.device, dtype=th.float32)
			diffused_actions = self.diffusion_policy(replay_data.observations, zero_input, return_numpy=False)
			diffused_actions = diffused_actions.reshape(-1, self.diffusion_act_chunk * self.diffusion_act_dim)
			res_actions = th.randn(replay_data.actions.shape[0], self.diffusion_act_chunk  * self.diffusion_act_dim).to(self.device)
			res_actions = th.tanh(res_actions)
			diffused_actions = (diffused_actions + res_actions).clamp(-1.0, 1.0)
			current_q_values = self.critic(replay_data.observations, diffused_actions)
	
		current_q_noise_vals = self.critic_noise(replay_data.observations, res_actions)
		critic_distill_loss = 0
	...
	def _sample_action(
		...
		if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
			...
			zero_input = th.zeros(action.shape[0], self.diffusion_act_chunk, self.diffusion_act_dim, device=self.device, dtype=th.float32)
			obs = th.as_tensor(self._last_obs, device=self.device, dtype=th.float32)
			...
			final_action = (action * self.res_scale + diffusion_action).clamp(-1.0, 1.0)
			final_action = final_action.cpu().numpy()
			buffer_action = final_action
	...
	def predict_diffused(
		...
		final_action = (action * self.res_scale + diffusion_action).clamp(-1.0, 1.0)
		final_action = final_action.cpu().numpy()
		return final_action, predict_second_return
